...
.PHONY: run
run: ## run the bot-tg
	go run ${LDFLAGS} cmd/bot-tg/main.go

.PHONY: run-restart
run-restart: ## restart the bot-tg
	@pkill -P `cat $(PID_FILE)` || true
	@printf '%*s\n' "80" '' | tr ' ' -
	@echo "Source file changed. Restarting bot-tg..."
	@go run ${LDFLAGS} cmd/bot-tg/main.go & echo $$! > $(PID_FILE)
	@printf '%*s\n' "80" '' | tr ' ' -
...
build: ## build the bot-tg binary
	CGO_ENABLED=0 go build ${LDFLAGS} -a -o bot-tg $(MODULE)/cmd/bot-tg

.PHONY: build-docker
build-docker: ## build the bot-tg as a docker image
	docker build -f cmd/bot-tg/Dockerfile -t bot-tg .

.PHONY: clean
clean: ## remove temporary files
	rm -rf bot-tg coverage.out coverage-all.out
